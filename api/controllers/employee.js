  ...
  async getLeaveAllocations(req, res) {
    try {
      ...
      const allocations = await EmployeeLeaveAllocation.find({
        ...
        .skip(startIndex)
        .limit(limit);
      const totalAllocations = await EmployeeLeaveAllocation.countDocuments({
        employee: req.params.id,
        isDeleted: false,
      });
    ...
    try {
      ...
      const query = {
        ...
        $or: [
          ...
          { responder: { $regex: searchKey, $options: "i" } },
        ],
      };
      const history = await EmployeeLeaveHistory.find({
        employee: req.params.id,
        isDeleted: false,
      })
        .populate({
      ...
      const totalPages = Math.ceil(totalLeaveHistory / limit);
      res.status(200).json({
        history,
        currentPage: page,
        totalPages,
